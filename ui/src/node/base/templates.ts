...
export function inputFromDT(dt: DT): Function | void {
	switch(dt) {
		...
		case DT.deg: return ({name, value, mode}: Field & Port) => html`<cam-input type="number" autosize
		...
		case DT.percent: return ({name, value, mode}: Field & Port) => html`<cam-input type="number" autosize
