...
import { Ndjinn } from '../../node/base/node-base.js'
import store, { DatatypeMap, setTransforms } from '../../store/store.js'
...
export const NdjinnConfig = define<any>({
  ...
  config: {
    ...
    observe: (host, config) => {
      ...
      config.nodes &&
        Object.entries(config.nodes).forEach(([name, [fn, defaults, options]]) => {
